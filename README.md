...
- **2025-05-30**：提示：还有一些功能还在开发，还要过几天才会发布：[Release: v2-alpha](https://www.bilibili.com/video/BV1GJ411x7h7)，先给各位更新一波数据库解解馋

- **2025-05-29**：实验性版本已更新，抢先体验：[>>> 点我访问 <<<](http://222.186.21.40:33333/)
...
- ☁️ 支持 [从私人网盘导出](#使用教程)（不创建分享链接、不会被审查）
...
- ⚡ 提供基于 `Flask` 的 [API 调用接口](docs/API_TUTORIAL.md)，提供公共资源库相关API（可对接Js脚本）、导入导出操作API（可对接机器人）
...
- 加入排队功能，避免因为多人使用导致服务器IP被封（请务必在单IP的服务器上设置**单进程**运行, 线程数可以随意）
