...
SHARE_PATH = os.path.abspath("./export")
SHARE_FILES = [_ for _ in os.listdir(SHARE_PATH) if _.endswith(".123share")]
for SINGLE_FILE in tqdm(SHARE_FILES):
    CURRENT_DATAS = []
    with open(os.path.join(SHARE_PATH, SINGLE_FILE), "r", encoding="utf-8") as f:
        _temp_datas = json.loads(base64.urlsafe_b64decode(f.read()))
